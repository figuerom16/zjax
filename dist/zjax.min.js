...
`,n,o.stack)}}function de(e){return async function(t){await le(e)(t)}}function le({handlerString:e}){const t=e.match(/^(?:(\w+)\.)?(\w+)$/);if(t){const n=t[1],o=t[2],r=zjax&&zjax.userActions;if(n){if(!(r[n]&&r[n][o]))throw new Error(`Unknown action: ${n}.${o}`);return r[n][o].bind(r[n])}else{if(!r[o])throw new Error(`Unknown action: ${o}`);return r[o].bind(r)}}try{return new Function("$",e)}catch(n){throw new Error(`z-action value is invalid: ${n.message}`)}}window.zjax=V(),u("Parsing DOM on document load"),w(document),y(document),document.addEventListener("turbo:load",()=>{u("Parsing DOM on turbo:load"),Q(),w(document),y(document)})})();
